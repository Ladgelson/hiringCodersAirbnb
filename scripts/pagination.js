      ...
      `<div class="row">
        <div class="col-md-7">
          <div class="acomodacao">
            <img class="img-card" src=` + item.photo + `>
            <div class="info">
              <div class="title-and-icon">
                <div class="card-title">
                  <strong>` + item.name + `</strong>
                </div>
                <div class="align-heart" onClick="like(`+ (5*page + i) +`)">
                  <img class="love-icon" 
                      src=` + selectIcon(item.liked) + `
                      id=`+ (5*page + i) + `
                      >
                </div>
              </div>
              <div><span>` + item.localidade + ", " + item.property_type + `</span></div>
              <div class="value">
                <span> <strong>R$` + item.price + `</strong>/noite</span>
              </div>
            </div>
          </div>
        </div>
...
function paginationButton(page, items) {
  ...
  button.addEventListener('click', () => {
    currentPage = page
    displayList(items, listElements, row, currentPage)
...
displayList(items, listElements, row, currentPage)
setupPagination(items, pagination, row)

function buscarLocalizacao() {
  let value = document.getElementById('localizacao').value
  console.log(value)
  value.toLowerCase()
  items = items.filter((item) => {
    return item.localidade.toLowerCase() == value
  })
  console.log(items)
  currentPage = 1
  displayList(items, listElements, row, currentPage)
...
function selectIcon(isLiked) {
  if (isLiked) return `"img/love-and-romance-fill.png"`
  return `"img/love-and-romance.png"`
}
...
function like(id){
  ...
  image.src = selectIcon(item.liked)
  displayList(items, listElements, row, currentPage)
}

function listFavorites(){
  items = items.filter((item)=> {return item.liked})
  currentPage = 1
  displayList(items, listElements, row, currentPage)
  setupPagination(items, pagination, row)
}
